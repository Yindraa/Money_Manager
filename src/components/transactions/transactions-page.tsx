...
export function TransactionsPage({ data }: { data: DashboardData }) {
  ...
  return <main className="min-h-screen bg-[#f3f5f1] text-slate-950">
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
      <label className="month-button mb-5 flex w-full sm:w-auto"><CalendarDays size={16} /><span className="sr-only">Pilih bulan transaksi</span><input type="month" min="2000-01" max="2099-12" value={monthValue} onChange={(event) => changeMonth(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none sm:w-[9rem] sm:flex-none" aria-label="Pilih bulan transaksi" /></label>
