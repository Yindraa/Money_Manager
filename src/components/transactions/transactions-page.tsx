...
export function TransactionsPage({ data }: { data: DashboardData }) {
  ...
  return <main className="min-h-screen bg-[#f3f5f1] text-slate-950">
    <header className="border-b border-slate-200/80 bg-[#f8faf7]/95 backdrop-blur-xl">
      <div className="mx-auto flex min-h-[76px] max-w-[1440px] flex-col items-stretch justify-between gap-3 px-4 py-3 sm:flex-row sm:items-center sm:px-7 lg:px-10">
        ...
        <label className="month-button w-full sm:w-auto"><CalendarDays size={16} /><input type="month" min="2000-01" max="2099-12" value={monthValue} onChange={(event) => changeMonth(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none sm:w-[9rem] sm:flex-none" aria-label="Pilih bulan transaksi" /></label>
      </div>
