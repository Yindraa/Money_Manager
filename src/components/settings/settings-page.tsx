...
import { BookOpen, CalendarDays, CreditCard, Eye, EyeOff, KeyRound, LayoutDashboard, ListFilter, LogOut, Plus, Save, Settings, Share2, Tag, UserRound, WalletCards } from "lucide-react";
...
export function SettingsPage({ data, email, categories, paymentMethods }: { data: DashboardData; email: string; categories: CategorySetting[]; paymentMethods: MethodSetting[] }) {
  ...
  function changeMonth(month: string) { if (month) router.push(`/settings?book=${data.book.id}&month=${month}`); }

  return <main className="min-h-screen bg-[#f3f5f1] text-slate-950">
    <header className="border-b border-slate-200/80 bg-[#f8faf7]/95 backdrop-blur-xl"><div className="mx-auto flex min-h-[76px] max-w-[1440px] items-center justify-between gap-3 px-4 py-3 sm:px-7 lg:px-10"><div className="flex min-w-0 items-center gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#123c32] text-white"><WalletCards size={21} /></div><div className="min-w-0"><select value={data.book.id} onChange={(event) => changeBook(event.target.value)} className="max-w-[10rem] bg-transparent text-[15px] font-semibold outline-none sm:max-w-[15rem]" aria-label="Pilih buku keuangan">{data.books.map((book) => <option key={book.id} value={book.id}>{book.name}</option>)}</select><p className="text-xs capitalize text-slate-400">{data.role} · Money Manager</p></div></div><label className="month-button hidden sm:flex"><CalendarDays size={16} /><input type="month" min="2000-01" max="2099-12" value={monthValue} onChange={(event) => changeMonth(event.target.value)} className="w-[9rem] bg-transparent text-sm font-semibold outline-none" aria-label="Pilih bulan" /></label></div><nav className="mx-auto flex max-w-[1440px] gap-1 overflow-x-auto px-4 sm:px-7 lg:px-10" aria-label="Navigasi utama"><Link href={`/?book=${data.book.id}&month=${monthValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-transparent px-3 py-3 text-sm font-semibold text-slate-500"><LayoutDashboard size={16} /> Dashboard</Link><Link href={`/transactions?book=${data.book.id}&month=${monthValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-transparent px-3 py-3 text-sm font-semibold text-slate-500"><ListFilter size={16} /> Transaksi</Link><Link href={`/settings?book=${data.book.id}&month=${monthValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-emerald-700 px-3 py-3 text-sm font-semibold text-emerald-800"><Settings size={16} /> Pengaturan</Link></nav></header>
