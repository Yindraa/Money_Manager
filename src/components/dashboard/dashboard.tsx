  ...
  return <main className="min-h-screen bg-[#f3f5f1] text-slate-950">
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.35fr_0.85fr]">
        <article className="panel min-h-[430px]"><div className="panel-header"><div><h2 className="panel-title">Tren pengeluaran bulanan</h2><p className="panel-subtitle">Perbandingan total pengeluaran dalam enam bulan terakhir</p></div><span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">6 bulan</span></div><div className="h-[340px] w-full pt-6">{data.monthlyExpenseTrend.some((item) => item.value > 0) ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={data.monthlyExpenseTrend} margin={{ left: 0, right: 8, top: 8 }}><defs><linearGradient id="monthlyExpenseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2f7d68" stopOpacity={0.3} /><stop offset="95%" stopColor="#2f7d68" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#edf0ec" /><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: "#94a3b8", fontSize: 12 }} dy={10} /><YAxis axisLine={false} tickLine={false} width={55} tick={{ fill: "#94a3b8", fontSize: 11 }} tickFormatter={formatCompactCurrency} /><Tooltip formatter={(value) => [formatCurrency(Number(value)), "Pengeluaran"]} /><Area type="monotone" dataKey="value" stroke="#2f7d68" strokeWidth={3} fill="url(#monthlyExpenseFill)" activeDot={{ r: 5, fill: "#123c32", stroke: "#ffffff", strokeWidth: 2 }} /></AreaChart></ResponsiveContainer> : <EmptyChart message="Grafik akan muncul setelah ada pengeluaran dalam enam bulan terakhir." />}</div></article>
