...
export function Dashboard({ data }: { data: DashboardData }) {
  ...
  const dailyData = Array.from(expenses.reduce((map, item) => map.set(item.dateIso, (map.get(item.dateIso) ?? 0) + item.amount), new Map<string, number>())).sort(([a], [b]) => a.localeCompare(b)).map(([date, value]) => ({ date, day: new Intl.DateTimeFormat("id-ID", { day: "numeric", month: "short", timeZone: "UTC" }).format(new Date(`${date}T00:00:00Z`)), value }));
  ...
  return <main className="min-h-screen bg-[#f3f5f1] text-slate-950">
    <header className="border-b border-slate-200/80 bg-[#f8faf7]/90 backdrop-blur-xl"><div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-4 sm:px-7 lg:px-10"><div className="flex min-w-0 items-center gap-3"><button className="icon-button lg:hidden" aria-label="Buka menu"><Menu size={20} /></button><div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#123c32] text-white"><WalletCards size={21} /></div><div className="min-w-0"><select value={data.book.id} onChange={(event) => changeBook(event.target.value)} className="max-w-[10rem] bg-transparent text-[15px] font-semibold outline-none sm:max-w-[15rem]" aria-label="Pilih buku keuangan">{data.books.map((book) => <option key={book.id} value={book.id}>{book.name}</option>)}</select><p className="text-xs capitalize text-slate-400">{data.role} · Money Manager</p></div></div><div className="flex items-center gap-2 sm:gap-3"><label className="month-button hidden sm:flex"><CalendarDays size={16} /><span className="sr-only">Pilih bulan</span><input type="month" min="2000-01" max="2099-12" value={monthInputValue} onChange={(event) => changeMonth(event.target.value)} className="w-[9rem] bg-transparent text-sm font-semibold capitalize outline-none" aria-label="Pilih bulan dashboard" /></label>{isOwner && <button onClick={() => setIsShareOpen(true)} className="secondary-button px-3 sm:px-4"><Share2 size={16} /><span className="hidden sm:inline">Bagikan</span></button>}<span className="grid size-10 shrink-0 place-items-center rounded-full bg-[#d9eadf] text-sm font-bold text-emerald-900">{initials}</span></div></div><nav className="mx-auto flex max-w-[1440px] gap-1 overflow-x-auto px-4 sm:px-7 lg:px-10" aria-label="Navigasi utama"><Link href={`/?book=${data.book.id}&month=${monthInputValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-emerald-700 px-3 py-3 text-sm font-semibold text-emerald-800"><LayoutDashboard size={16} /> Dashboard</Link><Link href={`/transactions?book=${data.book.id}&month=${monthInputValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-transparent px-3 py-3 text-sm font-semibold text-slate-500 hover:text-slate-900"><ListFilter size={16} /> Transaksi</Link><Link href={`/settings?book=${data.book.id}&month=${monthInputValue}`} className="inline-flex shrink-0 items-center gap-2 border-b-2 border-transparent px-3 py-3 text-sm font-semibold text-slate-500 hover:text-slate-900"><Settings2 size={16} /> Pengaturan</Link></nav></header>
    <div className="mx-auto max-w-[1440px] px-4 py-7 sm:px-7 lg:px-10 lg:py-10">
      <label className="month-button mb-5 flex w-full sm:hidden"><CalendarDays size={16} /><span className="sr-only">Pilih bulan</span><input type="month" min="2000-01" max="2099-12" value={monthInputValue} onChange={(event) => changeMonth(event.target.value)} className="min-w-0 flex-1 bg-transparent text-sm font-semibold outline-none" aria-label="Pilih bulan dashboard" /></label>
