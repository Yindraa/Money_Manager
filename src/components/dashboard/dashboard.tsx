...
export function Dashboard({ data }: { data: DashboardData }) {
  ...
  const monthlyCategoryTotals = Array.from(expenses.reduce((map, item) => {
    const current = map.get(item.category);
    map.set(item.category, { value: (current?.value ?? 0) + item.amount, color: item.categoryColor });
    return map;
  }, new Map<string, { value: number; color: string }>())).map(([name, item]) => ({ name, ...item })).sort((a, b) => b.value - a.value);
  const monthlyCategoryData = monthlyCategoryTotals.length > 6
    ? [...monthlyCategoryTotals.slice(0, 5), { name: "Lainnya", value: monthlyCategoryTotals.slice(5).reduce((sum, item) => sum + item.value, 0), color: "#94a3b8" }]
